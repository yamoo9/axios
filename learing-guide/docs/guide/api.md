---
title: API
description: Axios 라이브러리 API에 대해 학습합니다.
---
...
`axios()`에 url을 전달해 요청할 수 있습니다. 선택적으로 `config` 객체를 2번째 인자로 추가 전달할 수 있습니다.
...
동시(Concurrency) 요청 처리를 위한 헬퍼 함수
...
사용 가능한 인스턴스 메소드는 다음과 같습니다. 설정된 구성(config)이 인스턴스 구성과 병합 됩니다.
...
:::tip NOTE
`url` 속성만 필수이고, 나머지 속성은 옵션입니다.
`method`가 지정되지 않으면 요청은`GET`으로 기본 설정 됩니다.
:::
